...
async function research() {
    ...
    const cameraDiagIds = [
        ...
        'aPHUjtY8QFEKaVU5X0LG2vQ', // CCTV camera status
        'aOzdYMcJkw06ft9g4uXvpIA'  // Video device health
    ];

    console.log('Fetching ALL devices...');
    const allDevices = await call('Get', { typeName: 'Device' });
    const vehicles = allDevices.slice(0, 20);

    console.log(`Checking camera diagnostics for ${vehicles.length} vehicles...`);
    ...
    for (const v of vehicles) {
        ...
        const hasAny = results.some((r: any[]) => r && r.length > 0);
        if (hasAny) {
            console.log(`Vehicle ${v.name} (${v.id}) has camera data:`);
            results.forEach((r: any[], idx: number) => {
                if (r && r.length > 0) {
                    console.log(` - ${cameraDiagIds[idx]}: ${r[0].data} at ${r[0].dateTime}`);
                }
            });
        } else {
            // console.log(`Vehicle ${v.name} (${v.id}) NO camera data.`);
        }
    }
}
